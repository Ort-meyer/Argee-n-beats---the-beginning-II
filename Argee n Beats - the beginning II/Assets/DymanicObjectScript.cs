...
public class DymanicObjectScript : MonoBehaviour {
	...
	void Update ()
    {
        ...
        if (orbiScript.m_usingDragAbility)
        {
            ...
            m_goalPos = t_crossResultNorm * 2 + t_targetTransform.position;
            ...
            if(m_enterOrbit)
            {
                ...
                {
                    print(-1 * (t_targetTransform.position - gameObject.transform.position) * (Mathf.Log10(t_vectorBetween.magnitude / m_radius)) * 100);
                    gameObject.GetComponent<Rigidbody>().AddForce(1*(t_targetTransform.position - gameObject.transform.position) *(Mathf.Log10(t_vectorBetween.magnitude/m_radius))*100);
