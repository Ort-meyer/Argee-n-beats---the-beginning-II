...
public class TP_AimTrajectoryScript : MonoBehaviour {
    ...
    public void ToggleTrajectory(bool on)
    {
        ...
        if (on)
        {
            t_lineRenderer.positionCount = m_sections;
        }
        else
        {
            t_lineRenderer.positionCount = 0;
        }
	...
	public void Update () {
        ...
        ////List<Vector3>t_vecList =  RangeCombatScript.m_instance.GetTrajectory(m_endpoint, t_lineRenderer.positionCount);
        //for (int i = 0; i < t_lineRenderer.positionCount; i++)
        //{
